export function factory(L, PIXI) {
  ...
  var pixiOverlayClass = {
    ...
    initialize: function (drawCallback, pixiContainer, options) {
      ...
      this._doubleBuffering =
        PIXI.utils.isWebGLSupported() && !this.options.forceCanvas && this.options.doubleBuffering
    },
  ...
  if (L.version >= '1') {
    ...
    pixiOverlayClass._setContainerStyle = function () {
      var self = this
      ;['-webkit-transform-origin', '-ms-transform-origin', 'transform-origin'].forEach(
        function (property) {
